...
public func request(_ url: String) -> NormalBuilder {
    let builder = Manager.shared.request(url)
    return builder
}

public func upload(_ url: String) -> UploadBuilder {
    let builder = Manager.shared.upload(url)
    return builder
}
...
public class Manager {
    ...
    public static let shared = Manager(configuration: AFSession.default.session.configuration)
    ...
    public var configuration: URLSessionConfiguration = URLSessionConfiguration.default {
        didSet {
            ...
            afSession = AFSession(configuration: configuration, serverTrustManager: Manager.trustManager)
        }
    }
        ...
        didSet {
            if let item = proxyItem {
                if proxyItem != oldValue {
                    ...
                    afSession = AFSession(configuration: sessionConfiguration, serverTrustManager: Manager.trustManager)
                }
            } else {
                afSession = AFSession(configuration: AFSession.default.session.configuration, serverTrustManager: Manager.trustManager)
            }
        }
    }
    ...
    public func request(_ url: String) -> NormalBuilder {
        let builder = NormalBuilder(url: url, session: afSession, network: self)
        builder.querySorter = self.client?.commonParametersSorter
        return builder
    }
    
    public func upload(_ url: String) -> UploadBuilder {
        let builder = UploadBuilder(url: url, session: afSession, network: self)
        builder.querySorter = self.client?.commonParametersSorter
        return builder
    }
