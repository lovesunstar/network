...
public class NormalBuilder: HTTPBuilder {
    ...
    open override func build() -> Request? {
        ...
        if vappendCommonParameters {
            ...
            if let commonParameters = Manager.client?.commonParameters {
        ...
        if let defaultHeaders = Manager.client?.requestHeaders {
            headers += defaultHeaders
        }
        headers += vheaders
        var newURLString = absoluteString as String
        var postParameters = self.postParameters
        Manager.client?.willProcessRequest(&newURLString, headers: &headers, parameters: &postParameters)
        ...
        if let HTTPBody = encodedURLRequest.httpBody, let client = Manager.client {
